    ...
    for filename in os.listdir(input_folder):
        if filename.endswith(".txt"):
            ...
            with open(file_path, "r", encoding="utf-8") as f:
                yield f.read(), filename  # ✅ Yield text + filename

# ✅ Process the files with nlp.pipe() in batches
for text, filename in read_files():  # ✅ Read from generator correctly
    doc = nlp(text)  # ✅ Apply NLP model correctly

    output_path = os.path.join(output_folder, filename.replace(".txt", "_ner.json"))
    with open(output_path, "w", encoding="utf-8") as f:
        for token in doc:
            if token.text.strip() == "":  # ✅ Ignore empty tokens (e.g., newlines)
                continue
            token_data = {
