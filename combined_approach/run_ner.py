...
for doc, filename in nlp.pipe(read_files(), batch_size=1, as_tuples=True):
    global_id = 1  # ✅ Reset global_id for each document
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        for token in doc:
            ...
            if not token.text.strip():  # ✅ More robust whitespace handling
                continue
            token_data = {
                "global_id": global_id,  # ✅ Unique ID across the document
            ...
            global_id += 1  # ✅ Increment global_id for the next token
